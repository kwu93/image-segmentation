"""
Primary module for TensorFlow. Sets up the system, loss and training operations, placeholders for input and labels.
Takes in a model (CNN) and a config (for hyperparameters)
...
#        train_op = optimizer.apply_gradients(gradients_and_variables)
    ...
    def train(self, session, dataset, train_dir, sample=100, log=True):
        """ Set up some basic logging info """
        tic = time.time()
        ...
        """ We can also choose to save model params into our train_dir if we want """
        savePath = train_dir + "/model"
